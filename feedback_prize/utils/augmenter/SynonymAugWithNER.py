...
import utils.augmenter.base as augbase
...
class SynonymAugWithNER(augbase.WordAugmenterWithNER):
    # https://arxiv.org/pdf/1809.02079.pdf
    """
    似たような意味の単語で置き換えるAugmenter.以下のコードが必要?
    >>> !pip install nlpaug
    >>> import nltk
    ...
    Parameter
    ---------
    aug_src: 'wordnet' | 'ppdb'
        どのSynonym DBを用いるか。
    model_path: str
        PPDBを用いる際にどこにファイルを保存したかを指定する。http://nlpgrid.seas.upenn.edu/PPDB/eng/ppdb-2.0-tldr.gz
    ...
    tokenizer: func
        デフォルトでFeedback Prize用にホワイトスペースで分割するように変えてあります
    reverse_tokenizer: 
        Customize reverse of tokenization process
    force_reload: bool
        Force reload model to memory when initialize the class. Default value is False and suggesting to keep it as False if performance is the consideration.
    """

    def __init__(self, aug_src: str = 'wordnet', model_path=None, aug_min: int = 1, aug_max: int = 10, aug_p: float = 0.1, stopwords=None, tokenizer=split_tokenizer, reverse_tokenizer=None, stopwords_regex=None, force_reload=False):
        super().__init__(action=Action.SUBSTITUTE, name='Synonym_Aug', aug_p=aug_p, aug_min=aug_min, aug_max=aug_max, stopwords=stopwords, tokenizer=tokenizer, reverse_tokenizer=reverse_tokenizer, device='cpu', verbose=0, stopwords_regex=stopwords_regex, include_detail=False)

        self.aug_src = aug_src
        self.model_path = model_path
        self.lang = 'eng'
        self.model = self.get_model(aug_src, self.lang, model_path, force_reload)
